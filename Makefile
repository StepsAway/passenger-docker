...
.PHONY: build_all clean clean_images \
	build_ruby22 build_ruby23 build_jruby1726 \
	release test
...
clean_images:
	...
	@if docker images $(NAME)-jruby17:26-$(VERSION) | awk '{ print $$2 }' | grep -q -F 1-$(VERSION); then docker rmi -f $(NAME)-jruby17:26-$(VERSION) || true; fi
...
release: test
	...
	@if ! docker images $(NAME)-jruby17:26-$(VERSION) | awk '{ print $$2 }' | grep -q -F 1-$(VERSION); then echo "$(NAME)-jruby17:26-$(VERSION) is not yet built. Please run 'make build'"; false; fi
