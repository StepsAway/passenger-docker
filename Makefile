...
build_ruby242:
	...
	sed -i -e "s/##IMAGE##/ruby24:0-$(VERSION)/" ruby242_image/Dockerfile
...
test:
	...
	@if docker images $(NAME)-ruby24:2-$(VERSION) | awk '{ print $$2 }' | grep -q -F $(VERSION); then env NAME=$(NAME)-ruby24:1 RUBY='2.4.2' VERSION=$(VERSION) ./test/runner.sh; fi
